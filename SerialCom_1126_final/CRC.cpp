...
#define FCS(crc,data)		((crc << 8) ^ fcstbl[(crc >> 8) ^ data])
...
unsigned short CCRC::calc_crc(unsigned char *buf, int len)
{
...
int CCRC::check_crc(unsigned char *buf, int len)
{
	unsigned short crc = 0;
	while (len--) crc = FCS(crc, *buf++);
	return (crc==0);
}
